...
export default function StatePanel({remote, onSubmit}: Props) {
  ...
  return (
    <form className="p-4 rounded-lg max-w-md mx-auto" id="state-panel" onSubmit={submit}>
      ...
      <NumberInput name="swingDeg" label="Swing (deg)" binding={bind} min={0} max={360}/>
      <NumberInput name="elbowDeg" label="Elbow (deg)" binding={bind} min={0} max={360}/>
      <NumberInput name="wristDeg" label="Wrist (deg)" binding={bind} min={0} max={360}/>
